...
export async function POST(request: Request) {
  try {
    const { name, email, phone, position, department, role, password, branchId, companyId } = await request.json();
...
export async function PUT(request: Request) {
  try {
    const { id, name, email, phone, position, department, role, branchId } = await request.json();
